...
from apps.questionnaire.signals import create_questionnaire, delete_questionnaire, \
...
class ReceiverTest(TestCase):
    ...
    def _test_init_call(self, signal, log_type, **kwargs):
        with patch('apps.notifications.receivers.{}'.format(log_type)) as mock_status_log:
            getattr(signal, 'send')(
                sender='sender',
    ...
    def _test_create_call(self, signal, log_type, do_assert_call=True, **kwargs):
        ...
        with patch('apps.notifications.receivers.{}.create_log'.format(log_type)) as init:
            init.return_value = MagicMock()
            with patch('apps.notifications.receivers.{}.create'.format(log_type)) as create:
                getattr(signal, 'send')(
                    sender='sender',
