...
from apps.questionnaire.tests.test_views import valid_file
...
@patch('apps.questionnaire.upload.os')
class CreateThumbnailsTest(TestCase):

    @patch('apps.questionnaire.upload.subprocess')
...
@patch('apps.questionnaire.upload.os.makedirs')
class StoreFileTest(TestCase):
    ...
    @patch('apps.questionnaire.upload.magic.from_buffer')
    ...
    @patch('apps.questionnaire.upload.get_file_extension_by_content_type')
    def test_calls_get_file_extension_by_content_type(self, mock_func, mock_os):
        ...
        with patch('apps.questionnaire.upload.open') as mock_open:
            store_file(file)
    ...
    @patch('apps.questionnaire.upload.get_file_extension_by_content_type')
    @patch('apps.questionnaire.upload.sys.getsizeof')
    @patch('apps.questionnaire.upload.magic.from_buffer')
...
class RetrieveFileTest(TestCase):
    ...
    @patch('apps.questionnaire.upload.get_upload_folder_structure')
