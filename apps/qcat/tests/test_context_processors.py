...
from apps.qcat.context_processors import template_settings, MaintenanceAnnouncement
...
class QcatContextProcessor(TestCase):
    ...
    @patch('apps.qcat.context_processors.MaintenanceAnnouncement')
...
class MaintenanceAnnouncementTest(TestCase):
    ...
    @patch('apps.qcat.context_processors.cache')
    ...
    @patch('apps.qcat.context_processors.cache')
    @patch('apps.qcat.context_processors.messages')
    ...
    def test_flush_cache(self, mock_cache):
        ...
        with patch('apps.qcat.context_processors.open', mock_open(read_data='foo')):
            MaintenanceAnnouncement(self.request)
    ...
    def test_flush_file(self,):
        f = MagicMock()
        with patch('apps.qcat.context_processors.open', mock_open(mock=f)):
    ...
    def test_set_maintenance_overlay(self, mock_messsages, mock_cache):
        mock_cache.get = MagicMock(return_value=None)
        self.request.user = mommy.make(get_user_model())
        with patch('apps.qcat.context_processors.open', mock_open(read_data=self.when)):
            announcement = MaintenanceAnnouncement
